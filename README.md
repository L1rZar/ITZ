...
.
├── core/                 # Логика игры
│   ├── entities.py       # Character, ClampedInt, Hero/create_hero, Enemy/create_boss
│   ├── battle.py         # Боевая система (run_battle)
│   ├── game_mechanics.py # Эффекты + артефакты + инвентарь
│   ├── ui.py             # Терминалы + утилиты (wait_enter и т.п.)
│   ├── save_system.py    # Авторизация + сохранения (register_or_login, save/load/create state)
│   └── text_bank.py      # Все тексты сцен/глав
...
│   └── chapter_5.py
├── game/
│   └── main.py           # Точка входа, меню, прогон глав
└── data/
    ├── accounts.txt         # login:password
    ├── artifacts_pool.json  # пул артефактов
...
Пользователи/пароли: `data/accounts.txt` в формате `login:password`.
Сохранения: `data/players/<login>.json`.
